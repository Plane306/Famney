...
public class IncomeManager {
    ...
    private boolean incomeExistsOnDate(String familyId, String description, Date date) throws SQLException {
        String sql = "SELECT COUNT(*) FROM Income WHERE familyId=? AND description=? AND DATE(incomeDate)=? AND isActive=1";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    public Income getIncomeById(String incomeId) {
        String sql = "SELECT * FROM Income WHERE incomeId = ? AND isActive = 1";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, incomeId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public List<Income> getAllIncomes(String familyId) {
        ...
        String sql = "SELECT * FROM Income WHERE familyId = ? AND isActive = 1 ORDER BY incomeDate DESC";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, familyId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
